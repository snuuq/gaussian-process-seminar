...
\begin{frame}{Q\&A: Extension from Countable Dense Subsets}
    ...
    \begin{proof}
        ...
        \]
        Thus, \( f(x) = g(x), \forall x \in \mathcal{X} \).
...
\begin{frame}{Inference with Exact Function Evaluations}
    Suppose we observe $f$ at locations $\mathbf{x}$, revealing $\bm{\phi} = f(\mathbf{x})$. We know that \( p(\bm{\phi} | \mathbf{x}) = \mathcal{N}(\bm{\phi} ; \bm{\mu}, \bm{\Sigma}) \) so \(\kappa(x) = \text{cov} [\bm{\phi}, \phi | \mathbf{x}, x] \).
...
\begin{frame}{Inference with Noisy Function Evaluations}
    \begin{center}
        \begin{figure}
            \includegraphics[width = 11cm]{figure/fig-3-GP-moskedastic-noise.png}
            \caption{Conditioning GP on data corrupted by increasing levels of homoskedastic noise \cite{Garnett2023-gc}}
        \end{figure}
    \end{center}
\end{frame}
...
\begin{frame}{Interpretation of Posterior Moments: Vector Case}
    For vector-valued observations $\mathbf{y}$, factor the covariance as $C = SPS$ where:
    \begin{itemize}
        \item $S$: diagonal with $S_{ii} = \sqrt{C_{ii}} = \text{std}[y_i]$
        \item $P = \text{corr}[\mathbf{y}]$: observation correlation matrix
    \end{itemize}
    ...
    \begin{alertblock}{Posterior Moments (Vector Case)}
        \vspace{-0.5cm}
        \begin{align*}
            \text{Posterior mean of } \phi & : \mu + \sigma \bm{\rho}^\top P^{-1} \mathbf{z}     \\
            \text{Posterior std of } \phi  & : \sigma \sqrt{1 - \bm{\rho}^\top P^{-1} \bm{\rho}}
        \end{align*}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Q\&A: Differential Entropy as Global Uncertainty}
    ...
    \begin{block}{Definition: Differential Entropy}
        ...
        \]
    \end{block}
    \textbf{For Gaussian distributions}:
    \begin{itemize}
        \item Univariate: $H[\mathcal{N}(\mu, \sigma^2)] = \frac{1}{2}\log(2\pi e \sigma^2)$
        \item Multivariate: $H[\mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})] = \frac{1}{2}\log|2\pi e \boldsymbol{\Sigma}|$
    \end{itemize}

    \vspace{0.2cm}
    \textbf{For GPs}: The joint entropy of function values at locations $\mathbf{x}$ is:
    \[
        H[\boldsymbol{\phi} \mid \mathbf{x}] = \frac{1}{2}\log|2\pi e \, K(\mathbf{x}, \mathbf{x})|
    \]
\end{frame}

%------------------------------------------------

\begin{frame}{Q\&A: Why Differential Entropy Measures Global Uncertainty}
    \textbf{Key insight}: Entropy depends on the \alert{determinant of the covariance matrix}.

    \vspace{0.2cm}
    \textbf{Why ``Global''?}
    \begin{itemize}
        \item Marginal variance $\sigma_i^2 = \Sigma_{ii}$: uncertainty of \alert{individual} variables
        \item Determinant $|\boldsymbol{\Sigma}|$: \textbf{joint} uncertainty of \alert{all} variables together
        \item Also reflects correlations: high correlation $\Rightarrow$ smaller $|\boldsymbol{\Sigma}|$
    \end{itemize}
\end{frame}

%------------------------------------------------

\begin{frame}{Q\&A: Entropy Reduction in the Posterior}
    \textbf{Prior vs Posterior Covariance}:
    \begin{align*}
    ...
    \vspace{0.2cm}
    \begin{alertblock}{Global Uncertainty Reduction}
        For positive semidefinite matrices $A, B$: $|A| \leq |A + B|$, hence:
        \[
            \boxed{|K_\mathcal{D}(\mathbf{x}, \mathbf{x})| \leq |K(\mathbf{x}, \mathbf{x})|}
        \]
        Therefore, posterior entropy is \textbf{always $\leq$ prior entropy}.
    \end{alertblock}

    \vspace{0.2cm}
    \textbf{Interpretation}:
    \begin{itemize}
        \item Independent observations ($\bm{\rho} = 0$): no entropy change
        \item More precise observations (larger $C^{-1}$): greater entropy reduction $\Rightarrow$ more information gained
    \end{itemize}
\end{frame}
...
\begin{frame}{Q\&A: Information Theoretic Interpretation}
    \begin{exampleblock}{Question}
        Why does differential \textbf{entropy represent uncertainty}?
    \end{exampleblock}

    \vspace{0.3cm}
    \textbf{Definition of Entropy}:
    \begin{itemize}
        \item Entropy is defined as the \alert{expected surprise} (or information content)
        \item \textbf{Intuition}:
              \begin{itemize}
                  \item If an event is certain ($p(x) \approx 1$), surprise is 0.
                  \item If events are unpredictable, surprise is high.
              \end{itemize}
    \end{itemize}

    \vspace{0.2cm}
    Therefore, regardless of whether it is discrete or differential, \textbf{higher entropy} fundamentally implies \textbf{higher uncertainty} about the random variable's outcome.
\end{frame}
...
\begin{frame}{Q\&A: Designing Cross-Covariance Functions}
    ...
    \begin{enumerate}
        \item \textbf{Scaled base kernel}: $K_{fg}(x, x') = \rho \cdot K(x, x')$ where $|\rho| < 1$
              ...
              \end{itemize}
        \item \textbf{Linear Model of Coregionalization (LMC)} \cite{Alvarez2011-vl}:
              \[
...
\begin{frame}{Q\&A: Why Disjoint Union for Joint GP Domains?}
    ...
    \begin{itemize}
        \item Ensures each point $(x, i)$ uniquely identifies \textit{which} function (Check ref 22 in book)
              \begin{itemize}
                  \item A disjoint union represents a point $x \in \mathcal{X}_i$ by the pair $(x, i)$, thereby combining the domains while retaining their identities.
              \end{itemize}
        \item In previous example, if $\mathcal{X}_1 = \mathcal{X}_2 = [0, 30]$, we distinguish $f(10)$ from $g(10)$ by index $i$
